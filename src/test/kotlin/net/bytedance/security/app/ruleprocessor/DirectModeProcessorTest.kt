...
internal class DirectModeProcessorTest {
    init {
        SootHelper.initSoot(
            "DirectModeProcessorTest",
    ...
    fun createAnalyzers() {
        val rules = Rules(
            listOf(
                "${TestHelper.getTestClassSourceFileDirectory(this.javaClass.name)}/testdata/unZipSlipDirectMode.json"
            ), RuleFactory()
        )
        ...
        runBlocking {
            ...
            val analyzers = (rp as DirectModeProcessor).analyzers
            assertEquals(1, analyzers.size)
            analyzers.forEach {
                assertEquals(3, it.data.sourcePointerSet.size)
                assertEquals(3, it.data.sinkPointerSet.size)
            }
            analyzers.forEachIndexed { index, taintAnalyzer ->
                println("$index:")
                println(taintAnalyzer.dump())
            }
            SliceModeProcessorTest.mustContains(
                analyzers,
